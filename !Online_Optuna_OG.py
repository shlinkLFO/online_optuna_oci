...
# import matplotlib.pyplot as plt # Removed plotting for online script
import optuna
...
# ==============================================================================
def send_ntfy_notification(title, message, priority=4): # 5=max, 4=high, 3=default, 2=low, 1=min
    """Sends a notification via ntfy.sh."""
    if not NTFY_TOPIC:
        logging.warning("ntfy topic not configured. Skipping notification.")
        return

    # Ensure topic name is URL-safe (usually okay, but good practice)
    safe_topic = requests.utils.quote(NTFY_TOPIC)
    url = f"https://ntfy.sh/{safe_topic}"

    headers = {
        "Title": title.encode('utf-8'), # Encode headers to handle potential special chars
        "Priority": str(priority),
        "Tags": "chart_with_upwards_trend,sol" # Example tags (maps to emoji + filter tag)
    }

    try:
        # Encode message body as UTF-8
        response = requests.post(url, data=message.encode('utf-8'), headers=headers, timeout=15) # Increased timeout slightly
...
def fetch_latest_data(conn, symbol, table_name, limit):
    """Fetches the latest N rows for a given symbol."""
    ...
    try:
        ...
        if not data:
             ...
             return pd.DataFrame(columns=['timestamp', 'open', 'high', 'low', 'close', 'volumefrom', 'volumeto']) # Return empty DF
...
# ==============================================================================
# --- Feature Engineering Function (calculate_features_min_rare_event - Unchanged from last fix) ---
# ==============================================================================
# --- Feature Engineering Functions (Adding GPT's Binary Features) ---
...
def calculate_features_min_rare_event(df_input):
    ...
    (This function should be the corrected version from the previous step)
    """
    # ... (function remains the same as previous version where uppercase was fixed) ...
    df = df_input.copy()
    ...
    # --- TA FEATURES (RSI, Stoch, MACD, BBands) ---
    logging.info("  Calculating TA features...")
    try:
        min_ta_warmup = 30
        if len(df) >= min_ta_warmup:
            ...
            df.rename(columns={'MACDH_12_26_9': 'MACDH_12_26_9'}, inplace=True, errors='ignore')
             ...
             logging.warning(f"  Warning: Insufficient data ({len(df)} rows) for TA warmup ({min_ta_warmup}). Skipping TA features.")
             logging.info("  Standardizing base column names to uppercase (TA skipped).")
             df.columns = df.columns.str.upper()
    except Exception as e_ta:
        logging.error(f"!! Error calculating TA features: {e_ta}")
        logging.info("  Standardizing base column names to uppercase (TA failed).")
        df.columns = df.columns.str.upper()
        for col in ['RSI_14', 'STOCHK_14_3_3', 'MACDH_12_26_9', 'BBP_20_2.0', 'BBB_20_2.0']:
             if col not in df.columns: df[col] = np.nan

    # --- Now use UPPERCASE names for all subsequent calculations ---
    atr_base_col = f'ATR_{atr_periods_min[0]}M'
    df['ATR_NORM'] = df[atr_base_col] / (df['CLOSE'] + epsilon) if atr_base_col in df and 'CLOSE' in df else np.nan
    ...
    # ... (rest of derived TA, RSI divergence, interactions using UPPERCASE) ...
    ...
    if macd_hist_col_upper in df.columns and not df[macd_hist_col_upper].isnull().all():
         ...
         df['MACD_HIST_INCREASING'] = (macd_hist_numeric > macd_hist_numeric.shift(1)).astype(int)
    ...
    if rsi_col_upper in df.columns and not df[rsi_col_upper].isnull().all():
        for n_div in [30]:
            if len(df) > n_div and 'LOW' in df:
                min_price_n = df['LOW'].rolling(window=n_div, min_periods=n_div//2).min()
                min_rsi_n = df[rsi_col_upper].rolling(window=n_div, min_periods=n_div//2).min()
                price_lower_low = df['LOW'] < min_price_n.shift(1)
                rsi_higher_low = df[rsi_col_upper] > min_rsi_n.shift(1)
                valid_idx = price_lower_low.index[n_div-1:]
                if not valid_idx.empty:
                    df.loc[valid_idx, f'RSI_BULL_DIV_{n_div}M'] = (price_lower_low & rsi_higher_low).loc[valid_idx].astype(int)

    # --- Original Binary Interaction Features ---
    ...
    div_confirm_col = f'RSI_BULL_DIV_30M'
    if div_confirm_col in df and 'MACD_HIST_POSITIVE' in df:
         df['INT_DIV_CONFIRM_MACD'] = ((df[div_confirm_col].shift(1) == 1) & (df['MACD_HIST_POSITIVE'] == 1)).astype(int)
    ...
    # --- GPT Suggested Binary Interaction Features ---
    ...
    lag_col_gpt = 'LAG_5M_PRICE_RETURN'
    ...
    ma10_col = 'MA_10M'; ma30_col = 'MA_30M'; ma60_col = 'MA_60M'
    ...
    lag_col_vp_gpt = 'LAG_10M_PRICE_RETURN'
    ...
    range_col = 'PRICE_RANGE_PCT'; atr_norm_col = 'ATR_NORM'
    if range_col in df and atr_norm_col in df:
        ...
        df['GPT_BREAKOUT_VOL_ATR'] = ((df[range_col] > rolling_range_mean * 1.25) & (df[atr_norm_col] > rolling_atr_norm_mean) & rolling_range_mean.notna() & rolling_atr_norm_mean.notna()).astype(int)
    ...
    div_strong_col = 'RSI_BULL_DIV_30M'
    ...
    final_cols_to_drop = cols_to_drop_pre_upper
    ...
    if not numeric_cols.empty and df[numeric_cols].isin([np.inf, -np.inf]).any().any():
...
def objective(trial, X, y, fixed_params, cv_strategy):
    """Objective function using SMOTEENN + tuning scale_pos_weight, maximizing Precision@Threshold."""
    # ... (function remains the same as previous version) ...
    param = {
        ...
        "learning_rate":    trial.suggest_float("learning_rate", 0.015, 0.50, log=True),
    ...
    try:
        ...
        for fold, (train_idx, val_idx) in enumerate(cv_strategy.split(X_np, y_np)):
            ...
            if len(np.unique(y_train_fold)) >= 2:
                ...
                if minority_count >= 6 and majority_count >=6:
                    try:
                    ...
                    except Exception as e_smoteenn:
        ...
        average_score = np.mean(cv_scores) if cv_scores else 0.0
    except optuna.exceptions.TrialPruned:
        raise
    except Exception as e:
        logging.error(f"Error in Optuna trial {trial.number}, fold {fold}: {e}")
        return 0.0
    return average_score if not np.isnan(average_score) else 0.0

# ==============================================================================
# --- Main Online Prediction Loop --- UPDATED WITH NTFY CALL ---
# ==============================================================================
    ...
    while True:
        ...
        try:
            ...
            if df_raw_data is None:
                logging.error("Database fetch failed. Skipping cycle.")
                if conn:
            ...
            if df_raw_data.empty:
                ...
                if last_processed_timestamp is None or latest_available_timestamp > last_processed_timestamp:
                    ...
                    if 'CLOSE' in df_features.columns:
                        ...
                        df_features[target_col] = close_series.shift(-PREDICTION_WINDOW_MINUTES).sub(close_series).div(close_series.replace(0, np.nan)).mul(100)
                    else:
                    ...
                    if not final_feature_cols:
                        logging.error("No numeric features found after selection. Skipping.")
                        last_processed_timestamp = latest_available_timestamp
                        continue

                    logging.info(f"Using {len(final_feature_cols)} features.")

                    # Prepare data for the LATEST prediction
                    if not df_features.empty:
                        X_predict_latest = df_features[final_feature_cols].iloc[-1:].copy()
                        X_predict_latest.replace([np.inf, -np.inf], np.nan, inplace=True)
                        X_predict_latest.fillna(0, inplace=True)
                    else:
                    ...
                    cols_for_nan_drop = [target_col] + final_feature_cols if target_col in df_train_ready.columns else final_feature_cols
                    ...
                    logging.info(f"Dropped {rows_before_drop - rows_after_drop} rows with NaNs/Infs for training.")

                    if target_col not in df_train_ready.columns:
                        logging.error(f"Target column '{target_col}' missing after NaN drop. Skipping.")
                        last_processed_timestamp = latest_available_timestamp
                        continue

                    if len(df_train_ready) < TRAIN_WINDOW_MINUTES // 2:
                        logging.warning(f"Insufficient training data ({len(df_train_ready)} rows) after NaN drop. Skipping.")
                        last_processed_timestamp = latest_available_timestamp
                        continue
                    ...
                    if len(np.unique(y_train_binary)) < 2:
                        logging.warning("Only one class present in training data. Skipping.")
                        last_processed_timestamp = latest_available_timestamp
                        continue
                    ...
                    try:
                        ...
                        pruner_warmup = max(1, OPTUNA_CV_SPLITS - 1); pruner_min_trials = max(5, N_OPTUNA_TRIALS // 5)
                        pruner = optuna.pruners.MedianPruner(n_warmup_steps=pruner_warmup, n_min_trials=pruner_min_trials)
                        sampler = optuna.samplers.TPESampler(seed=int(time.time()))
                    ...
                    if best_params_step is None:
                        logging.error("Optuna did not produce best parameters. Skipping prediction.")
                        last_processed_timestamp = latest_available_timestamp
                        continue

                    final_model_params = {**XGB_FIXED_PARAMS, **best_params_step}
                    logging.info("Fitting final model on training data for prediction...")
                    model_final = xgb.XGBClassifier(**final_model_params)
                    try:
                        model_final.fit(X_train, y_train_binary, verbose=False)
                        if not X_predict_latest.empty and set(final_feature_cols) == set(X_predict_latest.columns):
                             ...
                             if prediction_proba >= NOTIFICATION_THRESHOLD:
                                 ...
                                 notif_title = f"{SYMBOL_NAME} Alert (Prob >= {NOTIFICATION_THRESHOLD})"
                                 notif_message = (
                                     f"SOL pred prob: {prediction_proba:.4f}\n"
                             ...
                             logging.error("Prediction input empty or columns mismatch. Cannot predict.")
        ...
        # --- Exception Handling & Finally block (Unchanged) ---
        except psycopg2.OperationalError as db_op_error:
            logging.error(f"Database Operational Error: {db_op_error}. Attempting reconnect.")
            if conn:
                try: conn.close(); logging.warning("Closed potentially problematic DB connection.")
                except Exception: pass
            conn = None; time.sleep(60)
            ...
            if conn and conn.closed == 0 and prediction_made:
                 try:
                     conn.close(); logging.info("DB connection closed after prediction cycle.")
                     conn = None
            ...
            logging.info(f"Cycle took {elapsed_time:.2f}s. Waiting {wait_time:.2f}s for next 5-min interval...")
...
if __name__ == "__main__":
    # --- Prerequisite Check ---
    if not NTFY_TOPIC:
        logging.error("NTFY_TOPIC is not set in the configuration. Notifications will be disabled.")
        # Optionally exit if notifications are critical:
        # import sys
        # sys.exit("Error: NTFY_TOPIC is required.")

    logging.info(f"--- Starting Online Prediction Script for {SYMBOL_NAME} (5-Minute Cycle) ---")
    logging.info(f"Fetching {MINUTES_TO_FETCH} minutes, Training on {TRAIN_WINDOW_MINUTES} minutes.")
    logging.info(f"Targeting a full run cycle every {TARGET_CYCLE_TIME_SECONDS} seconds.")
    logging.warning(f"Optuna configured for {N_OPTUNA_TRIALS} trials per cycle. Monitor resource usage.")
